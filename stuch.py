...
class InpyStuchError(StuchException):
    ...
    def throw(self):
        print("An Error had threw outside your stuch program")
        print("Please submit the following as an issue:")
        try:
...
# Chapters
class Chapter:
    def __init__(self, name, ctype = "null"):
        self.name = name
        self.ctype = ctype
        self.value = None
...
def sym_usepylib(args):
    ...
    lib_dir = os.path.join(exe_dir, "libs")
    ...
    for name in args:
        ...
        for p in (
            ...
            os.path.join(lib_dir, f"{name}.py"),
        ):
            if os.path.isfile(p):
                found = p
                break
        ...
        if not found:
            LibError(f"pylib '{name}' not found", "sym_usepylib").throw()

        spec = importlib.util.spec_from_file_location(name, found)
        mod = importlib.util.module_from_spec(spec)
        sys.modules[name] = mod
        spec.loader.exec_module(mod)

        if not hasattr(mod, "getReg"):
            LibError(f"pylib '{name}' missing getReg()", "sym_usepylib").throw()

        reg = mod.getReg()

        if "tasks" in reg:
            for tname, tfunc in reg["tasks"].items():
                tasks_func[tname] = tfunc
                tasks[tname] = Task(tfunc)

        if "syms" in reg:
            for sname, sfunc in reg["syms"].items():
                syms[sname] = sfunc
...
syms.update({
    ...
    "ifstamp": sym_ifstamp,
    "clrtsk": sym_clrtsk,
    "usepylib": sym_usepylib,
})

stamps = {}  # name -> int(恢复stamp的行数)

define_newCPT(StrChapter, "lang", "StuchkutV1.0.0")
